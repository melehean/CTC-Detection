...
class Model:
    ...
    def gather_test_results(self, model):
        ...
        test_precision = precision_score(self.test_true_results, predictions, average='weighted')
        self.test_precision_list.append(test_precision)

        test_recall = recall_score(self.test_true_results, predictions, average='weighted')
        self.test_recall_list.append(test_recall)

        test_f1_score = f1_score(self.test_true_results, predictions, average='weighted')
...
def run_all_models(classifiers, clfs_names, train_data, train_true_results, test_data, test_true_results):
    balanced_acc = []
    roc_auc = []
    precision = []
    recall = []
    ...
    for (clf_name, clf) in zip(clfs_names, classifiers):
        ...
        model_object = Model(model=clf, train_data=train_data, train_true_results=train_true_results,
                             test_data=test_data, test_true_results=test_true_results, fold_number=3,
    ...
    data = {"Balanced Accuracy": balanced_acc, "ROC AUC": roc_auc, "Precision": precision, "Recall": recall,
            "F1 score": f1}
    results = pd.DataFrame(data=data, index=clfs_names)

    print("Best balanced accuracy:", ', '.join(get_best_metrics(clfs_names, balanced_acc)))
